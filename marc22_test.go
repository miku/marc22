...
650 [ 0] [(a) Sermons, Scottish], [(y) 19th century.]`
	...
	if out != exp {
		t.Errorf("Returned fields did not match expected fields, expected: %v, got: %s\n", exp, out)
	}
	log.Printf("GetDataFields()")
}
...
650 [ 0] [(a) Unit testing.]`
	...
	r.DataFields = append(r.DataFields, *df)
	out := r.String()
	if out != exp {
		t.Errorf("Output record string %s did not match expected string", out)
	}
